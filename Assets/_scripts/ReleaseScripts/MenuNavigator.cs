...
namespace MissingComplete
{
	public class MenuNavigator : MonoBehaviour 
	{
		...
		public void OnProfileNameSubmitted(Text name)
		{
			...
			//Check if Male
			bool sexMale = male.isOn;
			SaveGameManager.Instance.CreateSaveGame(currentCreateIndex, name.text, sexMale);
		}


		public void OnProfileNameCancelled(bool hide)
		{
			if(hide) {
				HideInputMask();
			}
		...
		{
			if(TurboActivateChecker.Instance != null 
			   && TurboActivateChecker.Instance.Activated == false) {
				OnGameNotActivated();
				return;
			}
		...
		{
			inputMask.DOFade(0.0f, NAME_INPUT_ANIMATION_TIME).OnComplete(() => inputMask.gameObject.SetActive(false));
		}
