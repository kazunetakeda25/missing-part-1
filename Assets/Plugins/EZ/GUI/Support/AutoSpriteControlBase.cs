...
public abstract class AutoSpriteControlBase : AutoSpriteBase, IControl, IUIObject, IPackableControl
{
	...
	/// </summary>
	public UITextBase spriteText;			// Object that will display our text
	...
	public virtual string Text
	{
		...
		set
		{
			...
			if (spriteText == null)
			{
				...
				spriteText = UITextBase.Create(this);
			}
	...
	{
		get { return SpriteText.Alignment_Type.Center; }
	}

	public virtual SpriteText.Anchor_Pos DefaultTextAnchor
	{
		get { return SpriteText.Anchor_Pos.Middle_Center; }
	}

	public float TextOffsetZ
	{
		get { return textOffsetZ; }
		set
		{
			textOffsetZ = value;
			if (spriteText != null)
				spriteText.ZOffset = textOffsetZ;
		}
	}
	...
	public override void Hide(bool tf)
	{
		...
		if (!IsHidden() && tf)
		{
			if (_collider is BoxCollider && Application.isPlaying)
			{
				savedColliderSize = ((BoxCollider)_collider).size;
				((BoxCollider)_collider).size = Vector3.zero;
			}
		}
		else if (IsHidden() && !tf)
		{
			...
			{
				((BoxCollider)_collider).size = savedColliderSize;
			}
		}
	...
	public virtual void Copy(SpriteRoot s, ControlCopyFlags flags)
	{
		...
		if ((flags & ControlCopyFlags.Appearance) == ControlCopyFlags.Appearance)
		{
			...
			if (c._collider != null)
			{
				...
				if (_collider == null)
				{
					gameObject.AddComponent(c._collider.GetType());
					customCollider = c.customCollider;
				}

				if (_collider.GetType() == c._collider.GetType())
				{
					if (c._collider is BoxCollider)
					{
						if (_collider == null)
							gameObject.AddComponent(typeof(BoxCollider));

						BoxCollider bc1 = (BoxCollider)_collider;
						BoxCollider bc2 = (BoxCollider)c._collider;
						bc1.center = bc2.center;
						bc1.size = bc2.size;
					}
					else if (c._collider is SphereCollider)
					{
						if (_collider == null)
							gameObject.AddComponent(typeof(SphereCollider));

						SphereCollider sc1 = (SphereCollider)_collider;
						SphereCollider sc2 = (SphereCollider)c._collider;
						sc1.center = sc2.center;
						sc1.radius = sc2.radius;
					}
					else if (c._collider is MeshCollider)
					{
						if (_collider == null)
							gameObject.AddComponent(typeof(MeshCollider));

						MeshCollider mc1 = (MeshCollider)_collider;
						MeshCollider mc2 = (MeshCollider)c._collider;
					...
					else if (c._collider is CapsuleCollider)
					{
						if (_collider == null)
							gameObject.AddComponent(typeof(CapsuleCollider));

						CapsuleCollider cc1 = (CapsuleCollider)_collider;
						CapsuleCollider cc2 = (CapsuleCollider)c._collider;
					...
					if (_collider != null)
						_collider.isTrigger = c._collider.isTrigger;
				}
			}
			else if (Application.isPlaying) // Don't create a collider if we're in edit mode
			{
				...
				else
				{
					...
					{
						SphereCollider sc = (SphereCollider)_collider;
	...
	public virtual void UpdateCollider()
	{
		...
		if (!(_collider is BoxCollider) || IsHidden() || m_spriteMesh == null || customCollider)
			return;
	...
	public override bool DidChange(SpriteRoot s)
	{
		...
		if (textOffsetZ != c.textOffsetZ)
		{
			if (c.spriteText != null)
				c.spriteText.ZOffset = textOffsetZ;
			return true;
		}
