...
public class UISlider : AutoSpriteControlBase
{
	...
	public float Value
	{
		...
		set 
		{
			...
			// If we haven't started yet, don't let our 
			// default value override our assigned value:
			if (!m_started)
		...
		if(Application.isPlaying)
		{
			...
			if (!managed)
			{
				if (knob.spriteMesh != null)
					((SpriteMesh)knob.spriteMesh).material = _renderer.sharedMaterial;
			}
			else
			{
				if (manager != null)
			...
			knob.animations[3].SetAnim(states[5], 3);
			...
			if (!disableEmptyBar)
			{
				...
				if (!managed)
					emptySprite._renderer.sharedMaterial = _renderer.sharedMaterial;
				else
				{
					if (manager != null)
					{
