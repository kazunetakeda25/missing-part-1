...
public class UIListItemContainer : ControlBase, IUIListObject, IUIContainer
{
	...
	protected List<UITextBase> textObjs = new List<UITextBase>();
	...
	public void ScanChildren()
	{
		...
		UITextBase txt;
		Component[] txts = transform.GetComponentsInChildren(typeof(UITextBase), true);
	...
	public void AddChild(GameObject go)
	{
		...
		{
			UITextBase txt = (UITextBase)go.GetComponent(typeof(UITextBase));
			if (txt != null)
			{
				textObjs.Add(txt);
...
//				continue;
	...
	public void Delete()
	{
		...
		for (int i = 0; i < textObjs.Count; ++i)
		{
			if (textObjs[i] is SpriteText)
				((SpriteText)textObjs[i]).Delete();
		}
	}
