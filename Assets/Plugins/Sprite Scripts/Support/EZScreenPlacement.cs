...
public class EZScreenPlacement : MonoBehaviour, IUseCamera
{
	...
	void Awake()
	{
		...
		IUseCamera uc = (IUseCamera)GetComponent(typeof(IUseCamera));
#else
	...
	public void Start()
	{
		...
		if (renderCamera != null)
		{
			viewportPixels.x = renderCamera.pixelWidth;
			viewportPixels.y = renderCamera.pixelHeight;
		}
    ...
    public Vector3 ScreenPosToWorldPos(Vector3 screenPos) 
	{
		...
		switch (relativeTo.horizontal)
		{
			case HORIZONTAL_ALIGN.SCREEN_RIGHT:
				pos.x = renderCamera.pixelRect.xMax + pos.x;
				break;
			case HORIZONTAL_ALIGN.SCREEN_LEFT:
				pos.x = renderCamera.pixelRect.xMin + pos.x;
				break;
			case HORIZONTAL_ALIGN.SCREEN_CENTER:
				pos.x = renderCamera.pixelRect.xMin + viewportPixels.x * 0.5f + pos.x;
				break;
		...
		switch (relativeTo.vertical)
		{
			case VERTICAL_ALIGN.SCREEN_TOP:
				pos.y = renderCamera.pixelRect.yMax + pos.y;
				break;
			case VERTICAL_ALIGN.SCREEN_BOTTOM:
				pos.y = renderCamera.pixelRect.yMin + pos.y;
				break;
			case VERTICAL_ALIGN.SCREEN_CENTER:
				pos.y = renderCamera.pixelRect.yMin + viewportPixels.y * 0.5f + pos.y;
				break;
