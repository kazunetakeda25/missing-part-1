...
public class ManualSprite : SpriteBase
{
	...
	*/
	...
	static public ManualSprite Create(string name, Vector3 pos)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
		return (ManualSprite)go.AddComponent(typeof(ManualSprite));
	}
	...
	/// <returns>Returns a reference to the component.</returns>
	static public ManualSprite Create(string name, Vector3 pos, Quaternion rotation)
	{
		GameObject go = new GameObject(name);
		go.transform.position = pos;
...
public class SpriteMirror : SpriteRootMirror
{
	...
	public override void Mirror(SpriteRoot s)
	{
		...
		lowerLeftPixel = ((ManualSprite)s).lowerLeftPixel;
		pixelDimensions = ((ManualSprite)s).pixelDimensions;
	}
	...
	public override bool DidChange(SpriteRoot s)
	{
		if (base.DidChange(s))
			return true;
		if (((ManualSprite)s).lowerLeftPixel != lowerLeftPixel)
		{
			s.uvsInitialized = false;
			return true;
		}
		if (((ManualSprite)s).pixelDimensions != pixelDimensions)
		{
			s.uvsInitialized = false;
			return true;
		}
